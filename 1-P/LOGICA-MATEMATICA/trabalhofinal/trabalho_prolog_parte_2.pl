...
% questao a, todas as capitais: capital(X).

% questao b, capitais de cidades pequenas: cidadep(X).

% questao c todas as cidades grandes do norte.

regiao_estado(X,norte), cidadeg(X).

% questao d, todas as cidades cosmopolitas no sul.

regiao_estado(X, sul), cidadeg(X).

% questao e, todas as cidades cosmopolitas.
